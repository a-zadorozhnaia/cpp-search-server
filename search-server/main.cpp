...
private:
    ...
    vector<string> SplitIntoWordsNoStop(const string& text) const {
        ...
        for (const string& word : SplitIntoWords(text)) {
            if (!IsValidWord(word)) {
                throw invalid_argument("Наличие недопустимых символов (с кодами от 0 до 31) в тексте добавляемого документа"s);
            }
...
int main()
{
    //cout << "Исключения в конструкторе:"s << endl;
    try {
    ...
    //cout << "Исключения в AddDocument:"s << endl;
    try {
        SearchServer search_server("и в на"s);
    ...
    //cout << "Исключения в FindTopDocuments:"s << endl;
    try {
        SearchServer search_server("и в на"s);
    ...
    //cout << "Исключения в GetDocumentId:"s << endl;
    try {
        SearchServer search_server("и в на"s);
        vector<Document> documents;
